...
namespace FlagConsole.Drawing
{
    public class Rectangle : Shape
    {
        ...
        {
            Line line = new Line(this.Location, new Coordinate(this.Location.X + this.Size.Width - 1, this.Location.Y), this.Token);

            for (int y = this.Location.Y - 1; y < this.Location.Y + this.Size.Height - 1; y++)
            {
                line.StartPoint = new Coordinate(line.StartPoint.X, y + 1);
                line.EndPoint = new Coordinate(line.EndPoint.X, y + 1);
                line.Draw();
            }
        }
